...
import type { UsuarioFormDatos, Perfil, Usuario, NavegacionProps } from '../types';
...
export default function UsuariosLista({ setPagina }: NavegacionProps) {
  ...
  const onSubmit = async (datos: UsuarioFormDatos) => {
    try {
      if (esEdicion && usuarioIdEditando) {
        await api.post("/usuarios/actualizar", { id: usuarioIdEditando, ...datos }); // Actualizar
      } else {
        await api.post("/usuarios/crear", datos); // Crear
      }
        ...
        <div className="usuarios-lista-page">
          <div className="usuarios-header">
            ...
            <div className='usuarios-header-btns'>
              <button className="btn-link" onClick={() => setPagina('home')}>Volver al Home</button>
